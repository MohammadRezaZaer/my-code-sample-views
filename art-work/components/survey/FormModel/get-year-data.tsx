...
import RadioCheckbox from "@components/ui/forms/checkbox/radio-checkbox";
                    ...
                    <motion.div
                        ...
                        <ul className=' grid grid-cols-1 gap-2 lg:gap-4 flex-wrap relative'>
                            {currentQuest.type.items.map(item => {
                                let spanClass = values[currentQuest.type.name]["selections"].includes(item) ? ' !text-black2 !border-black2 active-item' : ''
                                    ...
                                    <motion.li key={item} onClick={(e) => {
                                        ...
                                        addItemi(item)
                                               ...
                                               className={cn('relative cursor-pointer' +
                                                   ' overflow-hidden text-lg relative p-2   rounded-[12px]' +
                                                   ' transition-colors duration-300 text-grey1', spanClass)}
                                    >
                                        <RadioCheckbox name={"cvb"} readOnly checked={spanClass.includes("active") && true}
                                                  label={item}/>
...
export default forwardRef(GetYearData)
