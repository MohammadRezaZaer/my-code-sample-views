...
const classes = {
  ...
  minimal:
    'bg-[#23224D] ps-10 pe-12 md:ps-14 border border-[#B3ABC9] focus:border-accent ',
};
...
const SearchBox: React.FC<Props> = ({
  ...
  return (
    <form onSubmit={onSubmit} className={cn('w-full', className)}>
      <div
        className={cn('rounded-full flex relative', {
      ...
      >
        ...
        <input
          ...
          autoComplete="off"
          className={cn(
            'w-full h-full flex item-center' +
              ' appearance-none transition duration-300' +
              ' ease-in-out  text-sm placeholder-gray-500 ' +
              'overflow-hidden rounded-full focus:outline-none' +
              ' focus:ring-0',
            classes[variant]
          )}
          {...rest}
        ...
        {value && (
          <button
            ...
            className={cn(
              'cursor-pointer h-full w-10 md:w-14' +
                ' flex items-center justify-center absolute' +
                ' text-body transition-colors duration-200 ' +
                ' focus:outline-none hover:text-accent-hover' +
                ' focus:text-accent-hover',
              {
