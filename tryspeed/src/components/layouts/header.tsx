...
import AuthTopMenu from "@components/layouts/menu/authorized-top-menu";

const Search = dynamic(() => import('@components/ui/search/search'));
const AuthorizedMenu = dynamic(() => import('./menu/authorized-menu'), {
    ssr: false,
});
const AppsMenu = dynamic(
    () => import("./menu/apps-menu"),
    {ssr: false}
);
...
const Header = () => {
    ...
    return (
        <header
            className={cn('site-header-with-search h-14 md:h-16 lg:h-22 w-full ', {
                'lg:!h-auto': isFlattenHeader,
            })}
        >
            <div
                className={cn(
                    'flex justify-between items-center h-14 md:h-16 lg:h-22 px-4 lg:px-8 py-5 z-50  shadow-sm transition-transform duration-300',
                    {
                        ' lg:bg-transparent lg:shadow-none lg:border-0':
                        isFlattenHeader,
                    }
                )}
            >
                ...
                <ul className="hidden lg:flex items-center flex-shrink-0 space-s-10 text-[#B6B6B6]">
                    <li>
                        <AppsMenu />
                    </li>
                    {isAuthorize&&<AuthTopMenu/>}
